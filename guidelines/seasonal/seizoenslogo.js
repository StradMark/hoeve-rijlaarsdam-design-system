...
if(field==='screen'){
 ...
 const defs=document.createElementNS(NS,'defs');defs.appendChild(symbol);layer.appendChild(defs);
 ...
 const build=()=>{
  ...
  let out='';
  for(let i=0;i<n;i++){
   const x=(i+.1+rnd()*.8)/n*W, y=-6-rnd()*14, s=6+rnd()*8;
   const t=(11+rnd()*12).toFixed(1), dx=(rnd()*150-75).toFixed(0), sw=(rnd()*24-12).toFixed(0);
   const rot=(rnd()*260-130).toFixed(0), op=(.34+rnd()*.5).toFixed(2);
